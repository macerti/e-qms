...
export function ActivityDetailPanel({
  ...
  const { getRequirementsForActivity, allRequirements } = useManagementSystem();
  
  const [isEditing, setIsEditing] = useState(false);
  const [editName, setEditName] = useState("");
  const [editDescription, setEditDescription] = useState("");
  const [allocatedRequirements, setAllocatedRequirements] = useState<Requirement[]>([]);
  const [initialAllocatedIds, setInitialAllocatedIds] = useState<Set<string>>(new Set());
  const [showUnsavedWarning, setShowUnsavedWarning] = useState(false);

  const isGovernance = activity?.id.startsWith(GOVERNANCE_ACTIVITY_ID_PREFIX) || false;

  // Check if there are unsaved requirement changes
  const hasUnsavedRequirementChanges = useMemo(() => {
    const currentIds = new Set(allocatedRequirements.map(r => r.id));
    if (currentIds.size !== initialAllocatedIds.size) return true;
    for (const id of currentIds) {
      if (!initialAllocatedIds.has(id)) return true;
    }
    return false;
  }, [allocatedRequirements, initialAllocatedIds]);
  ...
  useEffect(() => {
    if (activity) {
      ...
      // For non-governance activities, also load manually allocated requirements
      if (!isGovernance && activity.allocatedRequirementIds?.length) {
        const manuallyAllocated = allRequirements.filter(r => 
          activity.allocatedRequirementIds?.includes(r.id)
        );
        // Merge generic requirements (for display) with manually allocated
        const merged = [...reqs];
        manuallyAllocated.forEach(r => {
          if (!merged.find(existing => existing.id === r.id)) {
            merged.push(r);
          }
        });
        setAllocatedRequirements(merged);
        setInitialAllocatedIds(new Set(merged.map(r => r.id)));
      } else {
        setAllocatedRequirements(reqs);
        setInitialAllocatedIds(new Set(reqs.map(r => r.id)));
      }
    }
    setIsEditing(false);
  }, [activity, processId, getRequirementsForActivity, isGovernance, allRequirements]);
  ...
  const handleSaveRequirements = () => {
    // Persist requirement allocations to the activity
    // Only save non-generic requirements (generic are system-managed)
    const manuallyAllocatedIds = allocatedRequirements
      .filter(r => r.type !== "generic")
      .map(r => r.id);
    
    onUpdate(activity.id, {
      allocatedRequirementIds: manuallyAllocatedIds,
    });
    
    // Update initial state to reflect saved changes
    setInitialAllocatedIds(new Set(allocatedRequirements.map(r => r.id)));
  };

  const handleDiscardRequirementChanges = () => {
  ...
  const handleOpenChange = (newOpen: boolean) => {
    if (!newOpen && hasUnsavedRequirementChanges) {
        ...
        <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
          ...
          <div className="py-6 space-y-6">
            ...
            <section className="pt-4 border-t border-border">
              ...
              {!isGovernance && hasUnsavedRequirementChanges && (
                <div className="mt-4 p-3 bg-warning/10 border border-warning/30 rounded-lg">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button 
                      variant="ghost" 
                      size="sm" 
                      onClick={handleDiscardRequirementChanges}
                    >
                      Discard
                    </Button>
                  </div>
      ...
      <AlertDialog open={showUnsavedWarning} onOpenChange={setShowUnsavedWarning}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You have unsaved requirement allocations. Do you want to discard these changes?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
