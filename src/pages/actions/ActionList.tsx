...
export default function ActionList() {
  ...
  const filteredActions = useMemo(() => {
    return actions.filter((action) => {
      ...
      if (filterValues.issue === "with_issue" && (!action.linkedIssueIds || action.linkedIssueIds.length === 0)) return false;
      if (filterValues.issue === "without_issue" && action.linkedIssueIds && action.linkedIssueIds.length > 0) return false;
      if (searchValue) {
        const query = searchValue.toLowerCase();
        if (!action.title.toLowerCase().includes(query) && !action.code.toLowerCase().includes(query)) return false;
      }
      return true;
    });
            ...
            {filteredActions.map((action, index) => {
              ...
              return (
                <button
                  ...
                  className="action-card w-full text-left group animate-fade-in"
                  style={{ animationDelay: `${index * 30}ms` }}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                        <span className="font-mono text-xs text-action font-semibold bg-action/8 px-2 py-0.5 rounded-md">
                          {action.code}
                        </span>
                        <StatusBadge status={action.status} />
                        {overdue && (
                          <span className="flex items-center gap-1 text-xs text-destructive font-medium bg-destructive/8 px-2 py-0.5 rounded-md">
                            <AlertCircle className="w-3 h-3" />
                            Overdue
                          </span>
                        )}
                      </div>
                      <h3 className="font-semibold truncate text-[15px]">{action.title}</h3>
                  ...
                  <div className="flex items-center gap-3 mt-4 pt-3 border-t border-border/60 text-xs text-muted-foreground flex-wrap">
                    <span className={cn(
                      "flex items-center gap-1.5 px-2 py-1 rounded-md",
                      overdue ? "bg-destructive/8 text-destructive" : "bg-muted/60"
                    )}>
                      <Clock className="w-3.5 h-3.5" />
                      {format(new Date(action.deadline), "dd MMM yyyy")}
                    </span>
                    <span className="bg-muted/80 px-2.5 py-1 rounded-md font-medium">
